...
### 8.1. 请求去重与 Singleflight 机制

DDNS Gateway 内置了请求去重缓存 (Dedupe Cache) 和 Singleflight 机制，用于：

1. 避免重复请求：在缓存窗口内，相同请求直接返回缓存结果，不调用上游 API

2. 请求合并：多个并发的相同请求只会产生一次上游 API 调用

#### 8.1.1. 缓存键计算

缓存键由以下字段计算得出（SHA256 哈希）：

| 字段                       | 参与键计算 | 原因                                                                    |
|----------------------------|------------|-------------------------------------------------------------------------|
| `operation`                | Yes        | 区分 Upsert / Delete                                                    |
| `provider`                 | Yes        | 不同服务商                                                              |
| `zone`                     | Yes        | 不同域名                                                                |
| `record_type`              | Yes        | 不同记录类型                                                            |
| `record`                   | Yes        | 不同记录名                                                              |
| `value`                    | Yes        | 不同值                                                                  |
| `ttl`                      | Yes        | 不同 TTL                                                                |
| `comment`                  | Yes        | 不同备注                                                                |
| `proxied`                  | Yes        | Cloudflare 代理状态；<br>仅 Cloudflare 的 A / AAAA / CNAME 记录参与计算 |
| `upstream_credential_hash` | Yes        | 不同凭证应分开缓存                                                      |
| `gateway_token`            | No         | 网关认证与 DNS 操作无关                                                 |
| `Accept` / `format`        | No         | 响应格式不影响操作本身                                                  |

注意：

1. 只有当 `provider` 为 Cloudflare 且 `record_type` 为 A / AAAA / CNAME 时，`proxied` 字段才参与缓存键的计算

2. 上游凭证 (upstream credentials) 通过 SHA256 哈希后参与缓存键计算，确保：

    - 不同用户使用不同凭证访问相同记录时，会产生不同的缓存键

    - 用户 A 的认证错误（如 403）不会影响使用有效凭证的用户 B

    - 哈希不可逆，不会造成凭证泄露

#### 8.1.2. Singleflight

当多个相同请求并发到达时，系统使用 Leader / Waiter 模式确保只有一个请求实际调用上游 API：
...
```text
       Request A                   Request B                   Request C
           ...
           ▼                           ▼                           ▼
  ┌──────────────────┐        ┌──────────────────┐        ┌──────────────────┐
  │ mark_in_flight() │        │ mark_in_flight() │        │ mark_in_flight() │
  └────────┬─────────┘        └────────┬─────────┘        └────────┬─────────┘
           │                           │                           │
           ▼                           ▼                           ▼
    ┌──────────────┐           ┌───────────────┐           ┌───────────────┐
    ...
    └──────┬───────┘           └───────┬───────┘           └───────┬───────┘
           ...
           ▼                           ▼                           ▼
   ┌────────────────┐          ┌────────────────┐          ┌────────────────┐
   │ Becomes Leader │          │ Becomes Waiter │          │ Becomes Waiter │
   └───────┬────────┘          └───────┬────────┘          └───────┬────────┘
           │                           │                           │
           ▼                           │                           │
 ┌────────────────────┐                │                           │
...
└──────────┬───────────┘               │                           │
    ...
    │   result   │             │ cached result │           │ cached result │
    └────────────┘             └───────────────┘           └───────────────┘
```
...
1. 首个请求成为 Leader：
    ...
    - 调用 `mark_in_flight()` 成功（返回 `True`）

    - 负责实际调用上游 API

    - 完成后通过 `set()` 缓存结果，并通知所有 Waiter

2. 后续请求成为 Waiter：

    - 调用 `mark_in_flight()` 失败（返回 `False`）

    - 调用 `wait_for_result()` 等待 Leader 完成
...
```text
                 ...
                 Hit │                    │ Miss
                     ▼                    ▼
 ┌──────────────────────┐              ┌──────────────────────┐
 │ Return cached result │              │ 2. mark_in_flight()  │
 │ (success or error)   │              │ Try to become Leader │
 └──────────────────────┘              └──────────┬───────────┘
                                                  │
                                  ┌───────────────┴──────────────────────────────┐
                             True │                                              │ False
                 (Becomes Leader) │                                              │ (Leader exists / Lease valid)
                                  ▼                                              ▼
           ┌─────────────────────────┐                                       ┌───────────────────────┐
           │ 3(a). Call upstream API │                                       │ 3(b). Check lease age │
           │       (with retry)      │                                       │       Lease expired?  │
           └────────────┬────────────┘                                       └───────────┬───────────┘
                        │                                                                │
           ┌────────────┴────────────┐                                         ┌─────────┴─────────┐
...
┌──────────────────────┐   ┌────────────────────┐                              ▼                   ▼
│     cache.set()      │   │    cache.set()     │   ┌──────────────────────────────┐            ┌─────────────────────────┐
│ Cache success result │   │ Cache error result │   │ 4(a). mark_in_flight() again │            │ 4(b). wait_for_result() │
└──────────┬───────────┘   └─────────┬──────────┘   │       Become Leader          │            │       Wait for Leader   │
           ▼                         ▼              └──────────────┬───────────────┘            └────────────┬────────────┘
┌──────────────────────┐   ┌─────────────────────┐                 ▼                                         │
│ Return success result│   │ Return error result │          ┌──────────────┐              ┌──────────────────┼──────────────────┐
│    Notify waiter     │   │    Notify waiter    │          │ Same as 3(a) │      Success │          Timeout │                  │ Leader failed
└──────────────────────┘   └─────────────────────┘          └──────────────┘              ▼                  ▼                  ▼
                                                                             ┌───────────────┐    ┌──────────────────────┐    ┌────────────────────────────┐
                                                                             │ Return        │    │ Return timeout error │    │ Return leader failed error │
                                                                             │ cached result │    │ (can retry later)    │    │ (can retry)                │
                                                                             └───────────────┘    └──────────────────────┘    └────────────────────────────┘
```

**流程说明**：

1. **缓存查询**：首先检查是否有有效缓存，命中则直接返回
...
3. **Lease 过期检查**：
   - **已过期** → 执行 **takeover**（接管），再次调用 `mark_in_flight()` 成为新 Leader
   - **未过期** → 成为 Waiter，等待当前 Leader 完成

4. **Leader 执行**：
   - 调用上游 API（**注意：当前实现无自动重试机制**）
   - 成功或失败都会通过 `set()` 缓存结果

5. **Waiter 等待**：
   - `SUCCESS`：Leader 完成，返回缓存结果
   - `TIMEOUT`：等待超时，返回超时错误（可稍后重试）
   - `LEADER_FAILED`：Leader 异常失败且未缓存结果（极少见，仅紧急情况）

#### 8.1.4. 错误响应缓存

所有错误响应都会被缓存，这是设计的核心原则。例如：

| 错误类型 | 示例                     | 缓存行为 |
|----------|--------------------------|----------|
| 认证错误 | 401 / 403、凭证无效      | 缓存     |
| 权限错误 | 无权操作该域名           | 缓存     |
| 请求错误 | 参数无效、记录不存在     | 缓存     |
| 速率限制 | 429 Too Many Requests    | 缓存     |
| 上游超时 | 网络超时、连接失败       | 缓存     |
| 上游错误 | 5xx 错误（重试后仍失败） | 缓存     |

原因：

1. 上游调用已内置重试机制

    针对临时性网络错误（如 429、5xx、网络错误等）会自动重试。

    若重试仍失败，说明问题持续存在，短期内重试无意义。

2. 凭证隔离

    由于 Upstream Credentials 参与缓存键计算，若缓存键相同必定表示上游用户凭证相同。

    因此对于凭证无效、权限不足等错误，重试也不会成功。

当缓存命中时（无论成功还是错误），响应会包含：

1. `action: "deduped"`

    表示此响应来自缓存

2. `upstream_called: false`

    表示未实际调用上游 API

3. `warnings` 中包含一条 `code` 为 `DEDUPE_HIT_SHORTCIRCUIT` 的条目

    警告此响应来自缓存，同时在 `details` 中提供相关详细信息

#### 8.1.5. Waiter 的特殊情况处理

当 Waiter 等待 Leader 时，可能出现以下情况：

| 情况                        | Waiter 行为                      | 后续请求                 |
|-----------------------------|----------------------------------|--------------------------|
| Leader 成功                 | 返回缓存的成功结果               | 缓存命中                 |
| Leader 失败（上游错误）     | 返回缓存的错误结果               | 缓存命中（返回相同错误） |
| 等待超时（Leader 仍在处理） | 返回 `SINGLEFLIGHT_WAIT_TIMEOUT` | 可稍后重试               |

Leader 失败时会缓存错误结果，Waiter 和后续请求都会收到这个缓存的错误。这确保了：

- 不会对已知会失败的请求重复调用上游 API

- 缓存过期后，新请求会重新尝试（此时问题可能已解决）
...
| 配置项                      | 配置项                                 | 默认值                     | 说明                                  |
|-----------------------------|----------------------------------------|----------------------------|---------------------------------------|
| 是否启用                    | `dedupe.enabled`                       | 参见 `config.example.toml` | 是否启用缓存功能                      |
| 缓存窗口                    | `dedupe.window_seconds`                | 参见 `config.example.toml` | 缓存条目的有效期                      |
| 最大条目数                  | `dedupe.max_entries`                   | 参见 `config.example.toml` | 超出时按 LRU 策略淘汰                 |
| Waiter 等待超时时间（秒）   | `dedupe.singleflight_wait_timeout_sec` | 参见 `config.example.toml` | Waiter 等待 Leader 的最大时间         |
| Singleflight 租约时间（秒） | `dedupe.singleflight_lease_sec`        | 参见 `config.example.toml` | Leader 的最大处理时间，超时后可被接管 |
